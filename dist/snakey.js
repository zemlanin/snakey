...
(function() {
  var A, B, DOWN, ESC, LEFT, R, RIGHT, UP, canvasStream, directionStream, displayStream, fieldStream, hideStream, initialSnake, keyboardStream, lg, moveSnake, pauseStream, sadFace, showStream, snakeStream, statusStream, ticker, _ref,
    __slice = [].slice;
  ...
  showStream = keyboardStream.map(function(_arg) {
    var keyCode;
    keyCode = _arg.keyCode;
    return keyCode;
  }).bufferWithCount(10, 1).filter(function(keyCodes) {
    return _.isEqual(keyCodes, [UP, UP, DOWN, DOWN, LEFT, RIGHT, LEFT, RIGHT, B, A]);
  });
  ...
  showStream = showStream.flatMap(function(v) {
    return Rx.Observable.of(v);
  }).concat(keyboardStream.filter(function(_arg) {
    var keyCode;
    keyCode = _arg.keyCode;
    return keyCode === R;
  ...
  ticker = showStream.flatMap(function() {
    return Rx.Observable.just(-1).concat(Rx.Observable.interval(200)).map(function(index) {
      return index + 1;
    }).takeUntil(hideStream).takeUntil(pauseStream);
  });

  directionStream = keyboardStream.map(function(_arg) {
    var keyCode;
    keyCode = _arg.keyCode;
    return keyCode;
  }).filter(function(keyCode) {
    return keyCode === UP || keyCode === DOWN || keyCode === LEFT || keyCode === RIGHT;
  }).scan(DOWN, function(prev, next) {
    switch (next) {
      case UP:
      case DOWN:
  ...
  ticker.subscribe(function(index) {
    return statusStream.onNext(index);
  });
